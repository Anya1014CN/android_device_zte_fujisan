...
updated = re.sub(
    ...
    r'''(?s:.*?)'''
    r'''(?=    private int getVrDisplayMode\(\) \{\n)''',
    '',
    updated,
)
...
updated = re.sub(
    ...
    r'''                        syncFujisanSecondaryBacklight\(color & 0x000000ff\);\n'''
    r'''                    \}\n''',
    '\n',
    updated,
)
...
native_call_pattern = re.compile(
    r'''(                    setLight_native\(mId, color, mode, onMS, offMS,\n'''
    r'''                            brightnessMode(?:, mBrightnessLevel)?\);\n)''',
)
replacement = r'''\1
                    if (mId == LightsManager.LIGHT_ID_BACKLIGHT) {
